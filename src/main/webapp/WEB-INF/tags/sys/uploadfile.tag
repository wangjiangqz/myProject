...
<div id="fileupload-buttonbar${id}" class="fileupload-buttonbar" style="width: 360px;">  
    ...
    <div id="fileSpan${id}" class="btn btn-primary fileinput-button" >
        ...
        <span style="margin-left: 3px;" id="label${id}">${(buttonName == null or buttonName == '')?'资料上传':buttonName}</span>
    </div>
    <br/>
</div>
...
<script type="text/javascript">	
	    ...
	    $.ajax({
            ...
            success:function (data){  
                ...
                for(var i = 0;i < data.length;i++) {
                    ...
                    newBody.find("#delAd" + addId).attr("href", "${ctx}/base/tBaFile/downFile?id=" + fileId); 
                }
            }
         });
	}
	...
	var jqXHR = $("#file${id}").fileupload({
                ...
                formData: {name: "${id}", filesize: "${filesize}", isFilterEncryption : "${isFilterEncryption}"},       // 传递name
                ...
                //maxFileSize: 50 * 1024 * 1024,   // 文件大小，ie9不起作用
                    ...
                    if(isFilterEncryption == "yes" && result == "error"){
	                    ...
	                    if(1 != activeUploads || 0 == tempLength){  
	                        ...
	                        $("#fileupload-buttonbar" + id + " #newBody" + addId).find("#delAd" + addId).attr("href", "${ctx}/base/tBaFile/downFile?id=" + fileId);
	                        // 文件上传成功,存值处理
