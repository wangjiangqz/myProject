...
<%@ attribute name="smallName" type="java.lang.String" required="true" description="压缩文件的名称"%>
<%@ attribute name="value" type="java.lang.String" required="false" description="隐藏域值（ID）"%>
<%@ attribute name="smallValue" type="java.lang.String" required="false" description="压缩文件隐藏域值（ID）"%>
...
<div id="fileupload-buttonbar${id}" class="fileupload-buttonbar" >  
    <div id="fileSpan${id}" class="btn btn-success fileinput-button" style="background: #2576db">
        ...
        <input type="hidden" id="${id}Small" name ="${smallName}" value="${smallValue}" class="haveFileId"/>
   		...
   		<li class="span3" style="width: 160px;height: 100px;display: none;"  id="tbody${id}" >
        	<a id="liA" href="#" target="_blank" class="thumbnail" style="width: 130px; height:95px; float: left;" >
	            ...
	            <a id="imgA" href="#" style="color: red;float: none;"  >
	                <span class="icon-remove" onclick="delImageGz(this, '${id}', '${kind}')"></span>
	            </a>
...
<script type="text/javascript">	
	...
	function initImageFileGz(){
	    ...
	    var smallValueArray = smallValue.split(",");
	    
	    for(var i = 0;i < valueArray.length;i++) {
	        var tempValueArray = valueArray[i];
	        ...
	        $.ajax({
                url:"${ctx}/base/tBaFile/queryOneFile",
                async:false, 
                type:"post",
                data:{"value" : tempValueArray},
                dataType:"json",
                success:function (data){ 
                    ...
                    var fileId = data.id;
                    var smallFileId = smallValueArray[i];
                    ...
                    newBody.find("#imgA" + addId).attr("onclick", "delImageFileGz('" + fileId + "', '" + fid +  "', '" + kind + "', '" + smallFileId + "')");
	...
	// 页面删除文件的处理
	function delImageGz(obj, id, kind){
	    // 删除对应的文件
        if('more' != kind){//单附件
	...
	// 后台进行删除数据
	function delImageFileGz(fileId, id, kind, fileIdSmall){
	    //alert(fileId);// 文件id
	    $.ajax({
         ...
         if('more' != kind){//单附件
             ...
             var tempValueSmalls = $("#" + id + "Small").val().split(",");
             var tempValueSmall = "";
             for(var i = 0; i < tempValueSmalls.length; i++){
                 if(fileIdSmall != tempValueSmalls[i]){
                     if("" == tempValueSmall){
                         tempValueSmall = tempValueSmall + tempValueSmalls[i];
                     }else{
                         tempValueSmall = tempValueSmall + "," + tempValueSmalls[i];
                     }
                 }
             }
             $("#" + id + "Small").val(tempValueSmall);
         }
	}	

	var jqXHR = $("#file${id}").fileupload({
                autoUpload: true,                // 是否自动上传  
                url: "${ctx}/base/tBaFile/saveGz", // 上传地址  
                ...
                //maxFileSize: 50 * 1024 * 1024,   // 文件大小，ie9不起作用
                    ...
                    if(1 != activeUploads || 0 == tempLength){  
                        ...
                        $("#fileupload-buttonbar" + id + " #newBody" + addId).find("#imgA" + addId).attr("onclick", "delImageFileGz('" + fileId + "', '" + id +  "', '" + kind + "', '" + smallFileId + "')");
                        ...
                        if('more' != kind){//单附件
                            $("#" + id).val(fileId);
                            $("#" + id + "Small").val(smallFileId);
                        }else {// 多附件
                            $("#" + id).val($("#" + id).val()+"," + fileId);
                            $("#" + id + "Small").val($("#" + id + "Small").val() + "," + smallFileId);
                        }
                        $("#file" + id).fileupload("enable"); 
                    }
                },  
