	...
	<form:form id="inputForm" modelAttribute="TBaTempActivity" action="${ctx}/workflow/tBaTempActivity/save" method="post" class="form-horizontal">
		...
		<div class="form-group">
			<label class="control-label">
				<span name="tip" id="tip" style="color: red;font-size: 12px;">*</span>归属机构：
			</label>
			<div class="controls">
				<sys:treeselect id="company" name="company.id" value="${tBaTempActivity.company.id}"
								...
								url="/sys/office/treeData?type=1" cssClass="required form-control" notAllowSelectParent="true"
        ...
        <div class="form-group">
            ...
            <div class="controls">
                <form:input path="activityName" htmlEscape="false" maxlength="40" 
                    class="input-xlarge required checkLogo activityName form-control"/>
            </div>
        </div>
		<div class="form-group">
			<label class="control-label">
			...
			<div class="controls">
				<form:input path="logo" htmlEscape="false" maxlength="40" 
				    class="input-xlarge required checkLogo activityLogo form-control"/>
			</div>
		</div>
		<div class="form-group">
			<label class="control-label">状态：</label>
			<div class="controls">
			    <form:select path="status" class="input-small form-control">
		...
		<div class="form-group">
			...
			<div class="form-group" style="width: 100%;">
	                <label class="control-label" >流程节点定义表：</label>
	       </div>
			<div class="form-group" style="width: 100%;">
				<div >
					<table id="contentTable" class="table table-striped table-bordered table-condensed">
						<thead>
							<tr>
								<th class="hide"></th>
								<th style="width: 10%;">流程节点名称</th>
								<th style="width: 10%;">状态</th>
								<th style="width: 10%;">是否发送短信</th>
								<th style="width: 16%;">发送短信内容</th>
								<th style="width: 10%;">优先级</th>
								<th style="width: 15%;">备注</th>
								<th style="width: 20%;">角色分配</th>
								<shiro:hasPermission name="workflow:tBaTempActivity:edit">
								    <th width="10">操作</th>
								</shiro:hasPermission>
							</tr>
						...
						<tr id="tBaTempNoteList{{idx}}">
							...
							<td>
								<input id="tBaTempNoteList{{idx}}_name" name="tBaTempNoteList[{{idx}}].name" 
                                    type="text" value="{{row.name}}" maxlength="40" class="form-control input-small required "/>
							</td>
							<td>
                                <select id="tBaTempNoteList{{idx}}_status" name="tBaTempNoteList[{{idx}}].status" 
                                    data-value="{{row.status}}" class="form-control input-small required ">
                                    <option value=""></option>
							...
							<td>
								<select id="tBaTempNoteList{{idx}}_sendSms" name="tBaTempNoteList[{{idx}}].sendSms" data-value="{{row.sendSms}}" class="input-small required form-control">
                                    <option value=""></option>
							...
							<td>
								<textarea id="tBaTempNoteList{{idx}}_smsContent" name="tBaTempNoteList[{{idx}}].smsContent"
                                    rows="4" maxlength="4000" class="form-control">{{row.smsContent}}</textarea>
							</td>
							<td>
								<input id="tBaTempNoteList{{idx}}_priority" name="tBaTempNoteList[{{idx}}].priority" type="text" value="{{row.priority}}" maxlength="18" class="input-small required number form-control"/>
							</td>
							<td>
								<textarea id="tBaTempNoteList{{idx}}_remarks" name="tBaTempNoteList[{{idx}}].remarks" 
                                    rows="4" maxlength="250" class="form-control">{{row.remarks}}</textarea>
							</td>
                            <td>
