...
<body>
	...
	<form:form id="searchForm" modelAttribute="TBaPublicBean" action="${ctx}/base/tBaPublicBean/?tableConfigId=${tableConfigId}" method="post" class="breadcrumb form-search bg-gray-light callout">
			...
			<c:forEach items="${findDataList }" var="findData">
				<li class="formVal">
	                ...
	                <c:if test="${findData.controlType eq '05' or findData.controlType eq '06' }">
	                	<input name="${findData.fieldCode }_start" id="startTime" type="text" readonly="readonly"  class="input-medium form-control Wdate endDate required"
							value="<fmt:formatDate value="${map[start]}" pattern="yyyy-MM-dd HH:mm:ss"/>"
							onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>--
						<input name="${findData.fieldCode }_end" id="endTime" type="text" readonly="readonly"  class="input-medium form-control Wdate endDate required"
