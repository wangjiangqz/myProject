/**
 * Copyright &copy; 2019-2019 <a href="#">版权</a> All rights reserved.
 */
package com.lw.modules.base.entity;


import java.util.Date;
import java.util.List;

import com.lw.common.persistence.DataEntity;

/**
 * 功能测试数据录入Entity
 * @author weixm
 * @version 2016-04-15
 */
public class TBaPublicBean extends DataEntity<TBaPublicBean> {
	
	private static final long serialVersionUID = 1L;
	private String str1;
	private String str2;
	private String str3;
	private String str4;
	private String str5;
	private String str6;
	private String str7;
	private String str8;
	private String str9;
	private String str10;
	private String str11;
	private String str12;
	private String str13;
	private String str14;
	private String str15;
	private String str16;
	private String str17;
	private String str18;
	private String str19;
	private String str20;
	private String str21;
	private String str22;
	private String str23;
	private String str24;
	private String str25;
	private String str26;
	private String str27;
	private String str28;
	private String str29;
	private String str30;
	private String str31;
	private String str32;
	private String str33;
	private String str34;
	private String str35;
	private Date date1;
	private Date date2;
	private Date date3;
	private Date date4;
	private Date date5;
	private Date date6;
	private Date date7;
	private Date date8;
	private Date date9;
	private Date date10;
	private int num1;
	private int num2;
	private int num3;
	private int num4;
	private int num5;
	private String queryStr;
	private List<String> list;
	private List<TBaSonPublicBean> tbaBaSonPublicBeanList;
	public String getStr1() {
		return str1;
	}
	public void setStr1(String str1) {
		this.str1 = str1;
	}
	public String getStr2() {
		return str2;
	}
	public void setStr2(String str2) {
		this.str2 = str2;
	}
	public String getStr3() {
		return str3;
	}
	public void setStr3(String str3) {
		this.str3 = str3;
	}
	public String getStr4() {
		return str4;
	}
	public void setStr4(String str4) {
		this.str4 = str4;
	}
	public String getStr5() {
		return str5;
	}
	public void setStr5(String str5) {
		this.str5 = str5;
	}
	public String getStr6() {
		return str6;
	}
	public void setStr6(String str6) {
		this.str6 = str6;
	}
	public String getStr7() {
		return str7;
	}
	public void setStr7(String str7) {
		this.str7 = str7;
	}
	public String getStr8() {
		return str8;
	}
	public void setStr8(String str8) {
		this.str8 = str8;
	}
	public String getStr9() {
		return str9;
	}
	public void setStr9(String str9) {
		this.str9 = str9;
	}
	public String getStr10() {
		return str10;
	}
	public void setStr10(String str10) {
		this.str10 = str10;
	}
	public String getStr11() {
		return str11;
	}
	public void setStr11(String str11) {
		this.str11 = str11;
	}
	public String getStr12() {
		return str12;
	}
	public void setStr12(String str12) {
		this.str12 = str12;
	}
	public String getStr13() {
		return str13;
	}
	public void setStr13(String str13) {
		this.str13 = str13;
	}
	public String getStr14() {
		return str14;
	}
	public void setStr14(String str14) {
		this.str14 = str14;
	}
	public String getStr15() {
		return str15;
	}
	public void setStr15(String str15) {
		this.str15 = str15;
	}
	public String getStr16() {
		return str16;
	}
	public void setStr16(String str16) {
		this.str16 = str16;
	}
	public String getStr17() {
		return str17;
	}
	public void setStr17(String str17) {
		this.str17 = str17;
	}
	public String getStr18() {
		return str18;
	}
	public void setStr18(String str18) {
		this.str18 = str18;
	}
	public String getStr19() {
		return str19;
	}
	public void setStr19(String str19) {
		this.str19 = str19;
	}
	public String getStr20() {
		return str20;
	}
	public void setStr20(String str20) {
		this.str20 = str20;
	}
	public String getStr21() {
		return str21;
	}
	public void setStr21(String str21) {
		this.str21 = str21;
	}
	public String getStr22() {
		return str22;
	}
	public void setStr22(String str22) {
		this.str22 = str22;
	}
	public String getStr23() {
		return str23;
	}
	public void setStr23(String str23) {
		this.str23 = str23;
	}
	public String getStr24() {
		return str24;
	}
	public void setStr24(String str24) {
		this.str24 = str24;
	}
	public String getStr25() {
		return str25;
	}
	public void setStr25(String str25) {
		this.str25 = str25;
	}
	public String getStr26() {
		return str26;
	}
	public void setStr26(String str26) {
		this.str26 = str26;
	}
	public String getStr27() {
		return str27;
	}
	public void setStr27(String str27) {
		this.str27 = str27;
	}
	public String getStr28() {
		return str28;
	}
	public void setStr28(String str28) {
		this.str28 = str28;
	}
	public String getStr29() {
		return str29;
	}
	public void setStr29(String str29) {
		this.str29 = str29;
	}
	public String getStr30() {
		return str30;
	}
	public void setStr30(String str30) {
		this.str30 = str30;
	}
	public String getStr31() {
		return str31;
	}
	public void setStr31(String str31) {
		this.str31 = str31;
	}
	public String getStr32() {
		return str32;
	}
	public void setStr32(String str32) {
		this.str32 = str32;
	}
	public String getStr33() {
		return str33;
	}
	public void setStr33(String str33) {
		this.str33 = str33;
	}
	public String getStr34() {
		return str34;
	}
	public void setStr34(String str34) {
		this.str34 = str34;
	}
	public String getStr35() {
		return str35;
	}
	public void setStr35(String str35) {
		this.str35 = str35;
	}
	public Date getDate1() {
		return date1;
	}
	public void setDate1(Date date1) {
		this.date1 = date1;
	}
	public Date getDate2() {
		return date2;
	}
	public void setDate2(Date date2) {
		this.date2 = date2;
	}
	public Date getDate3() {
		return date3;
	}
	public void setDate3(Date date3) {
		this.date3 = date3;
	}
	public Date getDate4() {
		return date4;
	}
	public void setDate4(Date date4) {
		this.date4 = date4;
	}
	public Date getDate5() {
		return date5;
	}
	public void setDate5(Date date5) {
		this.date5 = date5;
	}
	public Date getDate6() {
		return date6;
	}
	public void setDate6(Date date6) {
		this.date6 = date6;
	}
	public Date getDate7() {
		return date7;
	}
	public void setDate7(Date date7) {
		this.date7 = date7;
	}
	public Date getDate8() {
		return date8;
	}
	public void setDate8(Date date8) {
		this.date8 = date8;
	}
	public Date getDate9() {
		return date9;
	}
	public void setDate9(Date date9) {
		this.date9 = date9;
	}
	public Date getDate10() {
		return date10;
	}
	public void setDate10(Date date10) {
		this.date10 = date10;
	}
	public int getNum1() {
		return num1;
	}
	public void setNum1(int num1) {
		this.num1 = num1;
	}
	public int getNum2() {
		return num2;
	}
	public void setNum2(int num2) {
		this.num2 = num2;
	}
	public int getNum3() {
		return num3;
	}
	public void setNum3(int num3) {
		this.num3 = num3;
	}
	public int getNum4() {
		return num4;
	}
	public void setNum4(int num4) {
		this.num4 = num4;
	}
	public int getNum5() {
		return num5;
	}
	public void setNum5(int num5) {
		this.num5 = num5;
	}
	public String getQueryStr() {
		return queryStr;
	}
	public void setQueryStr(String queryStr) {
		this.queryStr = queryStr;
	}
	public List<String> getList() {
		return list;
	}
	public void setList(List<String> list) {
		this.list = list;
	}
	public List<TBaSonPublicBean> getTbaBaSonPublicBeanList() {
		return tbaBaSonPublicBeanList;
	}
	public void setTbaBaSonPublicBeanList(
			List<TBaSonPublicBean> tbaBaSonPublicBeanList) {
		this.tbaBaSonPublicBeanList = tbaBaSonPublicBeanList;
	}
	public String getStrValue(String str){
		if("str1".equals(str)){
			return getStr1();
		}else if("str2".equals(str)){
			return getStr2();
		}else if("str3".equals(str)){
			return getStr3();
		}else if("str4".equals(str)){
			return getStr4();
		}else if("str5".equals(str)){
			return getStr5();
		}else if("str6".equals(str)){
			return getStr6();
		}else if("str7".equals(str)){
			return getStr7();
		}else if("str8".equals(str)){
			return getStr8();
		}else if("str9".equals(str)){
			return getStr9();
		}else if("str10".equals(str)){
			return getStr10();
		}else if("str11".equals(str)){
			return getStr11();
		}else if("str12".equals(str)){
			return getStr12();
		}else if("str13".equals(str)){
			return getStr13();
		}else if("str14".equals(str)){
			return getStr14();
		}else if("str15".equals(str)){
			return getStr15();
		}else if("str16".equals(str)){
			return getStr16();
		}else if("str17".equals(str)){
			return getStr17();
		}else if("str18".equals(str)){
			return getStr18();
		}else if("str19".equals(str)){
			return getStr19();
		}else if("str20".equals(str)){
			return getStr20();
		}else if("str21".equals(str)){
			return getStr21();
		}else if("str22".equals(str)){
			return getStr22();
		}else if("str23".equals(str)){
			return getStr23();
		}else if("str24".equals(str)){
			return getStr24();
		}else if("str25".equals(str)){
			return getStr25();
		}else if("str26".equals(str)){
			return getStr26();
		}else if("str27".equals(str)){
			return getStr27();
		}else if("str28".equals(str)){
			return getStr28();
		}else if("str29".equals(str)){
			return getStr29();
		}else if("str30".equals(str)){
			return getStr30();
		}else if("str31".equals(str)){
			return getStr31();
		}else if("str32".equals(str)){
			return getStr32();
		}else if("str33".equals(str)){
			return getStr33();
		}else if("str34".equals(str)){
			return getStr34();
		}else if("str35".equals(str)){
			return getStr35();
		}
		return null;
	}
	
	public Date getDateValue(String str){
		if("date1".equals(str)){
			return getDate1();
		}else if("date2".equals(str)){
			return getDate2();
		}else if("date3".equals(str)){
			return getDate3();
		}else if("date4".equals(str)){
			return getDate4();
		}else if("date5".equals(str)){
			return getDate5();
		}else if("date6".equals(str)){
			return getDate6();
		}else if("date7".equals(str)){
			return getDate7();
		}else if("date8".equals(str)){
			return getDate8();
		}else if("date9".equals(str)){
			return getDate9();
		}else if("date10".equals(str)){
			return getDate10();
		}
		
		return null;
	}
	
	public Integer getIntValue(String str){
		if("num1".equals(str)){
			return getNum1();
		}else if("num2".equals(str)){
			return getNum2();
		}else if("num3".equals(str)){
			return getNum3();
		}else if("num4".equals(str)){
			return getNum4();
		}else if("num5".equals(str)){
			return getNum5();
		}
		return null;
	}
	

}