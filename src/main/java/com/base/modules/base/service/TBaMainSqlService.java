...
import com.base.modules.base.dao.TBaFieldDao;
import com.base.modules.base.dao.TBaMainSqlDao;
...
public class TBaMainSqlService extends CrudService<TBaMainSqlDao, TBaMainSql> {
	...
	public Map<String, Object> checkIsSql(String headSql,String tableName){
		JSONObject json = new JSONObject();
		Map<String, Object> map = new HashMap<String, Object>(1);
		//此处只需要达到sql的校验功能，不需要查询结果数量个，所以加上1 = 2 
		headSql = replaceString(headSql);
		String headSqlCount = "select count(*) from (" + headSql + " )" ;
		...
		try {
			...
			map = getResultTypeList(headSql,tableName);
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 */
	@SuppressWarnings("unchecked")
	public Map<String, Object> getResultTypeList(String headSql,String tableName){
		JSONObject json = new JSONObject();
		Map<String, Object> map = new HashMap<String, Object>(1);
        //创建视图
		...
		headMap.put("tableName1", tableName.toUpperCase());//此处一定要注意大写，因为数据库存储的表名均为大写
        ...
        for (TBaField tBaField : tBaFields) {
			...
			jsonInfo.put("comments", tBaField.getComments());
			jsonInfo.put("dataDefault", tBaField.getDataDefault());
	...
	public void createTempView(String headSql){
        ...
        createViewSql = "create or replace view v_temp_main_sql AS " + createViewSql + " ";
